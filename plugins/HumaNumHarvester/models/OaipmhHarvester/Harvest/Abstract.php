...
abstract class OaipmhHarvester_Harvest_Abstract
{
    ...
    private function _harvestLoop($record)
    {
        ...
        $collectionMetadata['elementTexts'] = $harvestedRecord['collectionMetadata']['elementTexts'];
        
        $collectionName = $collectionMetadata['elementTexts']['Dublin Core']['Title'][0]['text'];
        $this->_addStatusMessage($collectionName);
        $this->_addStatusMessage($this->_getCollectionIdByName($collectionName));
        
        if (!$this->_getCollectionIdByName($collectionName) && strlen(trim($collectionName))) {
            $collection = $this->_insertCollection($collectionMetadata);
            $harvestedRecord['itemMetadata']['collection_id'] = $collection->id;
        } else if(strlen(trim($collectionName))) {
            $harvestedRecord['itemMetadata']['collection_id'] = $this->_getCollectionIdByName($collectionName);
        }
    ...
    final protected function _insertItem(
        ...
        if (isset($fileMetadata['files'])) {
            ...
            $fileOptions = isset($fileMetadata['file_ingest_options']) 
                         ...
                         : array('ignore_invalid_files' => true); // Huma-num
            ...
            foreach ($files as $file) {
                ...
                $file = array("source"=>$file, "name"=>$fileMetadata['label']); // Huma-num : pass original_filename
                ...
                // If the file is about a Nakala harvest
                if ($fileMetadata['file_from_nakala_harvest']) {

                    // If the file is marked to delete (image, text or pdf file downloaded on the server)
                    if ($fileMetadata['delete_file_after_insert']) {

                        $cmd = "rm " . $fileMetadata['files'];
                        shell_exec($cmd);

                    } else { // Video or audio file not donwloaded

                        // Adding metadata from Virtuoso
                        if (get_class($fileOb[0]) == 'File') {
     ...
     * @return File|null
     */
    private function _getCollectionIdByName($collectionName = '') 
    {
